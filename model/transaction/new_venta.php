<?php
    ...
    if(isset($_SESSION['id_user'])){
        ...
        VALUE ($user, $id_cripto, $id_wallet, 'VENTA', $cant_cripto, $cant_pesos,'$date' , 1)")
         or die("Error ". mysqli_error($conexion));

         if($new_op){
                $update = $conexion -> query("UPDATE wallet_user SET OS_balance = (OS_balance + $cant_pesos ) WHERE ID_user = $user")
                or die("Error ". mysqli_error($conexion));
                echo 1;
        }else{
